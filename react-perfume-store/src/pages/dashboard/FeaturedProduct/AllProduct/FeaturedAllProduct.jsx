...
const FeaturedAllProduct = () => {
  ...
  const fetchProducts = async (page = 1) => {
    try {
      const res = await axios.get(`${BASE_URL}/featuredProduct?page=${page}&limit=10`);
      setLoading(false)
  ...
  const DelteProduct = async (id) => {
    ...
    if (result.isConfirmed) {
      try {
        await axios.delete(`${BASE_URL}/featuredProduct/${id}`, { withCredentials: true });
