...
exports.GetProducts = async (req, res) => {
  try {
    ...
    let query = FeaturedProduct.find(filter);
    ...
    const totalProducts = await FeaturedProduct.countDocuments(filter);
...
exports.UpdateProduct = async (req, res) => {
  try {
    ...
    const updatedProduct = await FeaturedProduct.findByIdAndUpdate(req.params.id, updateData, { new: true, runValidators: true });
...
exports.GetProduct = async (req,res)=>{
    try{
        const GetProduct = await FeaturedProduct.findById(req.params.id)
        if(!GetProduct){
